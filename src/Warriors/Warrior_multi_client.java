...
public abstract class Warrior_multi_client {
	public int HP;
	...
	public Warrior_multi_client(int Type, int Party,int ProductionID){
	...
	public boolean move(){
		...
		if (this.Party == WorldProperty.RED && this.Location < WorldProperty.NumberOfCity + 1  ){
			City_multi_client CurrentCity =  World_multi_client.CityList.get(Location);
			City_multi_client NextCity = World_multi_client.CityList.get(Location+1);
			NextCity.addWarrior(this);
			CurrentCity.removeWarrior(this);
			Location++;
			if (Location == WorldProperty.NumberOfCity + 1){
				Headquarters_multi_client RedHeadquarters = (Headquarters_multi_client) World_multi_client.CityList.get(Location);
		...
		if (this.Party == WorldProperty.BLUE && this.Location > 0){
			...
			Location--;
			if (Location == 0){
				Headquarters_multi_client BlueHeadquarters = (Headquarters_multi_client) World_multi_client.CityList.get(Location);
	...
	public boolean attack(Warrior_multi_client Object){
		...
		System.out.format("%s %s attacked %s in city %d with %d elements and force %d\n", 
				World_multi_client.WorldClock.getTime(),this.WarriorNameCard, Object.WarriorNameCard, this.Location,
				this.HP, this.AttackValue);
		
		//Give an attack to Object
		ClassAttack_multi_client myAttack = new ClassAttack_multi_client(this,Object,this.AttackValue,false);
		Object.getAttack(myAttack);
		
		//record this battle.
		if (this.Dead == false && Object.Dead == false){
			World_multi_client.CityList.get(this.Location).PartyOfLastRoundWinner = -1; 
		}
	...
	//Modify this and Wolf.java
        public static Warrior_multi_client deadWarrior = null;
	...
	public boolean getDeathAnnounce(ClassDeathAnnounce_multi_client da){
		...
		if (this instanceof Dragon_multi_client && this.Party == World_multi_client.CityList.get(this.Location).activeAttackParty){
			((Dragon_multi_client)this).cheers();
		}
	...
	protected void tryToChangeFlag() {
		City_multi_client myCity = World_multi_client.CityList.get(this.Location);
		if (myCity.PartyOfLastRoundWinner == this.Party){
			if (myCity.Flag != this.Party){
				...
				System.out.format("%s %s flag raised in city %d\n", 
						World_multi_client.WorldClock.getTime(), WorldProperty.PartyNames[this.Party],this.Location);
			}
			//Keep the flag.
		...
		if (removeHP(myAttack.attackValue)){
			...
			if (myAttack.isBeatBack == false){
				...
				System.out.format("%s %s fought back against %s in city %d\n", 
						World_multi_client.WorldClock.getTime(), this.WarriorNameCard, 
						myAttack.Subject.WarriorNameCard, this.Location);
				
				
				ClassAttack_multi_client myBeatback = new ClassAttack_multi_client(this,myAttack.Subject,this.AttackValue/2,true);
	...
	public boolean die(){
		...
		City_multi_client c = World_multi_client.CityList.get(this.Location);
