...
public class City_multi {
	public int CityID;
	public int LifeElement;
	public ArrayList<Warrior_multi> RedWarriorStation, BlueWarriorStation;
	public int Flag;
	public int PartyOfLastRoundWinner;
	public boolean Status_AfterBattle;
	public int activeAttackParty;
	
	public City_multi(int id){
		CityID = id;
		LifeElement = 0;
		Flag = -1;
		RedWarriorStation = new ArrayList<Warrior_multi>();
		BlueWarriorStation = new ArrayList<Warrior_multi>();
	...
	public boolean addWarrior(Warrior_multi w){
		switch (w.Party){
	...
	public boolean removeWarrior(Warrior_multi w){
	...
	public boolean organizeBattle(){
		...
		Warrior_multi RedWarrior = this.RedWarriorStation.get(0);
		Warrior_multi BlueWarrior = this.BlueWarriorStation.get(0);
		...
		if (Status_AfterBattle == true){
			if (this.RedWarriorStation.size()+this.BlueWarriorStation.size() == 1){
				if (!this.RedWarriorStation.isEmpty()){
					Headquarters_multi RedHeadquarters = (Headquarters_multi) World_multi.CityList.get(0);
					...
					Headquarters_multi BlueHeadquarters = (Headquarters_multi) World_multi.CityList.get(WorldProperty.NumberOfCity+1);
