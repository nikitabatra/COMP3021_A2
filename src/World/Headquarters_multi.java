...
import Warriors.Dragon_multi;
import Warriors.Iceman_multi;
import Warriors.Lion_multi;
import Warriors.Ninja_multi;
import Warriors.Warrior_multi;
import Warriors.WarriorType;
import Warriors.Warrior_multi;
import Warriors.Wolf_multi;

public class Headquarters_multi extends City_multi{
	...
	public Headquarters_multi(int[] order, int party, int initLifeElements, int cityID){
		super(cityID);
	...
	public Warrior_multi getNewArrivedWarrior(){
	 ...
	 */
	public boolean rewardWarrior(Warrior_multi Object,int RewardNumber){
	...
	public void rewardArmy(){
		...
		if (this.Party == WorldProperty.RED){
			for (int i=WorldProperty.NumberOfCity; i>=1; i--){
				City_multi c = World_multi.CityList.get(i);
...
//			System.out.println("Not enought Life Element");
	...
	private boolean produceWarrior(int warriorType) {
		...
		System.out.println("boom1");
				w = new Dragon_multi(warriorType, this.Party, this.ProductionID);
				break;
			case 1:
				w = new Ninja_multi(warriorType, this.Party, this.ProductionID);
				break;
			case 2:
				w = new Iceman_multi(warriorType, this.Party, this.ProductionID);
				break;
			case 3:
				w = new Lion_multi(warriorType, this.Party, this.ProductionID);
				break;
			case 4:
				w = new Wolf_multi(warriorType, this.Party, this.ProductionID);
				break;
			default:
				w = null;
