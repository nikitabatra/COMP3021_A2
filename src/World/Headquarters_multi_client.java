...
import Warriors.Dragon_multi_client;
import Warriors.Iceman_multi_client;
import Warriors.Lion_multi_client;
import Warriors.Ninja_multi_client;
import Warriors.WarriorType;
import Warriors.Warrior_multi_client;
import Warriors.Wolf_multi_client;

public class Headquarters_multi_client extends City_multi_client{
	...
	public Headquarters_multi_client(int[] order, int party, int initLifeElements, int cityID){
		super(cityID);
	...
	public Warrior_multi_client getNewArrivedWarrior(){
	 ...
	 */
	public boolean rewardWarrior(Warrior_multi_client Object,int RewardNumber){
	...
	public void rewardArmy(){
		...
		if (this.Party == WorldProperty.RED){
			for (int i=WorldProperty.NumberOfCity; i>=1; i--){
				City_multi_client c = World_multi_client.CityList.get(i);
	...
	public boolean tryToProduceWarrior(){
		...
		int warriorType = ProductionOrder[WarriorIndex];
                ...
                if(this.getParty() == WorldProperty.RED){
                    whichRedWarrior = World_multi.clientWarriorChosen;
                }
...
//			System.out.println("Not enought Life Element");
	...
	private boolean produceWarrior(int warriorType) {
		...
		Warrior_multi_client w;
		switch (warriorType){
			case 0:
                System.out.println(warriorType + " "+ this.Party + " "+ this.ProductionID);            
		System.out.println("boom1");
				w = new Dragon_multi_client(warriorType, this.Party, this.ProductionID);
				break;
			case 1:
				w = new Ninja_multi_client(warriorType, this.Party, this.ProductionID);
				break;
			case 2:
				w = new Iceman_multi_client(warriorType, this.Party, this.ProductionID);
				break;
			case 3:
				w = new Lion_multi_client(warriorType, this.Party, this.ProductionID);
				break;
			case 4:
				w = new Wolf_multi_client(warriorType, this.Party, this.ProductionID);
				break;
			default:
				w = null;
